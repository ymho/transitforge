...
describe("explicit conversation draft save", () => {
  it("creates before linking and only accepts the server read-back reference", async () => {
    const client = { create: vi.fn(async (trip) => trip), attach: vi.fn(async () => undefined) };
  ...
  it("keeps the same id for a response-loss retry and never creates a second Trip", async () => {
    ...
    const input = { conversationId, tripId, now, client: { create, attach: vi.fn(async () => undefined) }, readBack: async () => ({ tripId }) };
  ...
  it("does not delete the created Trip when linking fails", async () => {
    ...
    await expect(createConversationDraftTrip({ conversationId, tripId, now, client: { create, attach }, readBack: async () => undefined })).rejects.toThrow("link unavailable");
