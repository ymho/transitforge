...
interface RecheckDependencies { weather(location: string): Promise<{ forecast: { status: TravelRecheckSnapshot["status"]; data?: unknown; evidence: TravelRecheckSnapshot["evidence"] } }>; flights?(origin: string, destination: string, departureDate: string): Promise<{ flights: { status: TravelRecheckSnapshot["status"]; data?: unknown; evidence: TravelRecheckSnapshot["evidence"] } }>; railOperation?(serviceDate: string): Promise<{ data: unknown; evidence: TravelRecheckSnapshot["evidence"] }> }
...
async function recheck(item: StoredRecheck, dependencies: RecheckDependencies, now: Date): Promise<TravelRecheckSnapshot> {
  try {
    ...
    if (item.request.kind === "flight" && dependencies.flights) { const [route, departureDate] = item.request.entityId.split("@"); const [origin, destination] = (route ?? "").split("-"); if (origin && destination && departureDate) return snapshot((await dependencies.flights(origin, destination, departureDate)).flights, now); }
...
function label(kind: StoredRecheck["request"]["kind"]): string { return ({ weather: "天気", "rail-operation": "鉄道運行", flight: "航空便", "place-hours": "営業時間" })[kind]; }
