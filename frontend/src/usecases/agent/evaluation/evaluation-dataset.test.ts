...
describe("multi-turn dataset v2", () => {
  it("keeps the 42 six-metric cases and A–N, adding transport O/P", () => {
    ...
    expect(data.travelProgressScenarios).toHaveLength(16);
    expect(data.travelProgressScenarios?.filter((c) => c.tags.includes("smoke")).map((c) => c.id)).toEqual(["O-taxi", "N-unknown-child-age", "A-vague", "C-candidate", "G-consecutive", "K-food"]);
  });
