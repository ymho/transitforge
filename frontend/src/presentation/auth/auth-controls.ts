...
export function mountAuthControls(root: ParentNode, auth: AuthSession): () => void {
  ...
  const render = (state: AuthState) => {
    const text = state.status === "signed-in" ? state.displayName : {
    ...
    if (notice) { notice.hidden = state.status !== "error" && state.status !== "expired"; notice.textContent = notice.hidden ? "" : text; }
    if (login) { login.hidden = state.status === "signed-in" || state.status === "unavailable"; login.disabled = state.status === "authenticating"; }
    if (logout) logout.hidden = state.status !== "signed-in";
