...
- model routing比較では各caseのTraceと同じ実行のEval reportから `npm run eval:agent:model-routing:build -- --strategy <name> --report <report.json> --traces <traces.json> --output <run.json>`で`agent-model-routing-run-v2`を作る。単一modelと候補routingのrunを `npm run eval:agent:model-routing -- --baseline <single.json> --candidate <routing.json>`で比較する。`agent-model-routing-comparison-v3`の`productionRoutingRecommended`は同じdataset・case数・反復数と全品質指標の維持を前提に、品質の実測改善、またはmodel/tool callを増やさない10%以上のlatency/token改善を要求する。設定変更は自動実行しない
