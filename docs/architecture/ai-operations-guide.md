...
| 経路検索 | 営業日ごとの直通列車インデックスから直通または乗換1回の最大3件を検索 |
...
- 非公開S3の`direct-service-index-v1`をLambdaで検索
- 直通候補と乗換1回の候補を同じ列車データから組み立て 最新遅延を発着時刻へ反映する
- 乗換時間は既定5分とし 駅固有の例外は探索モジュールへ集約する
- 順位付けでは乗換1回へ10分相当の負担を加えるが 案内する発着時刻と待ち時間は変更しない
- `timetable-connection-index-v1`は乗換2回以上の探索を再開するときのために維持する
- 出発駅省略時の最寄り駅選択は端末内で行う
- 表示時計を候補列車の発車時刻へ移動しない
- 深夜帯の発着時刻はブラウザで通常時刻へ整形

検索ごとに`journey_search_trace`を構造化ログへ出す
出発駅の列車数 第1区間と第2区間の走査数 乗換時間不足 直通候補数 乗換候補数
採用経路 乗換駅 待ち時間を確認できる
