...
日程未確定の新しい旅行相談では発見段階に写真 Web調査と追加質問だけを公開し 旅程検討へ進んだ後に
日程不足を判定できるToolを公開して 実行上限へ到達させない。既存旅程への明示的な追加依頼では
変更案と追加確認のToolだけを公開し 新規の経路検索へ誤ルーティングさせない。
...
既存旅程で他の観光スポットや酒蔵などの施設種別を相談された場合は 目的地と日程を維持し
Web検索 ページ確認 POI照合だけへToolを絞る。自治体名 交通手段 施設種別そのものは候補から除外する。
