...
同一入力の重複実行抑止は維持する。ただし、資料未取得など、別Toolの結果で変化する
前提不足は`precondition_failed`として恒久的な入力エラーと分離する。
別のToolが成功したときだけ、その前提不足のキャッシュと失敗回数を解除し、モデルが
同じ入力で再評価できるようにする。モデルを待たずにToolを自動再実行するものではない。
成功済みの結果、恒久的な失敗、他のエラーによるTool除外、実行回数・timeoutは維持する。
同じ前提不足を状態変化なしに繰り返した場合は従来どおり抑止する。
