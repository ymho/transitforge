...
[Trip Progress評価](docs/architecture/trip-progress-evaluation.md)（A〜N）を追加実行する。
TTFC/TTFI・候補選択→draft・質問のみの連続数は構造化された表示成果物から測る。
SmokeはTrip Progress A/C/G/K/N、FullはA〜N。K/LはActivity、M/Nは[TripParty](docs/architecture/trip-party.md)の回帰評価。保存済み観測42件の6指標は維持する。
