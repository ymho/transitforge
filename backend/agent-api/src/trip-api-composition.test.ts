...
describe("public Trip API host", () => {
  it("requires an Access Token and exposes only the Trip writer route", async () => {
    const host = createTripApiPublicHandler({ enabled: true, auth: { userPoolId: pool, clientId: "app-client", requiredScopes: [scope] }, tripTable: "trips", verifier: cognitoTokenFixture().verifier });
