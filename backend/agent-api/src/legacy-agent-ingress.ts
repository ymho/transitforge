...
// Public map weather contains no personal state and invokes no model or paid provider.
const publicOperations = new Set(["weather_grid_search", "weather_forecast_search"]);
...
export function authorizeLegacyAgentRequest(verifier: AccessTokenVerifier) {
  ...
  return async (event: LambdaHttpEvent, value: JsonObject): Promise<void> => {
    if (typeof value.operation !== "string" ||
        !publicOperations.has(value.operation) && !protectedOperations.has(value.operation)) {
    ...
    if (publicOperations.has(value.operation)) return;
