      ...
      model: { converse: async () => {
        ...
        return ++calls === 1
          ...
          : { stopReason: "end_turn", metadata: { modelId: "fake", latencyMs: 0 }, message: { role: "assistant", content: [{ text:
            '<decision_summary>{"interpretedGoal":"天気確認","hardConstraints":[],"softPreferences":[],"selectedAction":"answer","unresolvedFacts":[],"reasonCodes":["information_missing"],"usedEvidenceIds":[]}</decision_summary>取得結果を案内します。' }] } };
      } },
