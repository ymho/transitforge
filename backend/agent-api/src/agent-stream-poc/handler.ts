...
export type ObservedAgentRun = (input: ServerAgentTurn, emit: AgentTurnEventSink) => Promise<void>;

/** Experimental POST endpoint. Authorizer claims/body identity never establish the principal. */
export function createAgentStreamHandler(options: { verifier: AccessTokenVerifier; run: ObservedAgentRun; newRunId: () => string; heartbeatMs?: number }) {
  ...
  return async (request: StreamRequest, writer: StreamWriter): Promise<void> => {
    ...
    try {
      if (request.method !== "POST" || request.path !== "/api/agent-stream-poc") throw new InputError(404);
      ...
      await options.run({ ...input, principal }, emit);
