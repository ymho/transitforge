...
import type { RailImpactRouter } from "../ports/rail-impact-routing.js";
...
export class DynamoDbRailImpactRouter implements RailImpactRouter {
  ...
  async route(subject: Extract<WatchSubject, { type: "rail-service" }>) {
    validateWatchSubject(subject);
    if (subject.type !== "rail-service") throw new TripResourceError("invalid-input");
    ...
    const routes: Awaited<ReturnType<RailImpactRouter["route"]>>["routes"] = [], seen = new Set<string>();
    let stale = 0;
