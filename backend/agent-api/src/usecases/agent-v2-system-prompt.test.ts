...
describe("agentV2SystemPrompt", () => {
  it("separates the user request, reference clock, accepted conditions and submitted reply", () => {
    for (const term of ["userMessage", "application.effectiveIntent", "clock", "read Tool", "structured output", "receiptId", "commentary", "自然な説明", "set_destination", "set_origin", "set_party", "clear_destination", "clear_party"])
