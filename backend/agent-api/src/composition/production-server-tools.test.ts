...
it("restores a Trip without Profile, invokes fixed-egress through the existing operation, and persists the grounded final", async () => {
  ...
  const converse = vi.fn(async () => converse.mock.calls.length === 1
    ...
    : { output: { message: { role: "assistant", content: [{ text: JSON.stringify({ responseText: "候補の宿を確認しました。空室は未確認です。", decision: { interpretedGoal: "宿を調べる", hardConstraints: [], softPreferences: [], selectedAction: "answer", unresolvedFacts: [], reasonCodes: ["evidence_sufficient"], usedEvidenceIds: ["observation:execution:accommodation:search_accommodations:q-8232276c:accommodation:travel-provider:42"] } }) }] } }, stopReason: "end_turn" });
