...
export function createInTripContextHandler(application?: Pick<InTripContextApplication, "read">, authenticate?: TripPrincipalResolver) {
  return async (event: LambdaHttpEvent) => {
    ...
    try {
      ...
      if (httpMethod(event) !== "POST" || !event.body || event.body.length > 512 || event.isBase64Encoded) throw new TripResourceError("invalid-input");
      const v = JSON.parse(event.body);
      requirePersonalOperation("inTrip", event, v);
      ...
      const authError = authenticationErrorResponse(e, "in-trip-api-v1");
