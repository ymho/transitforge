...
export type TravelRecheckKind = "weather" | "rail-operation" | "place-hours";
export interface TravelRecheckRequest { id: string; tripId: string; kind: TravelRecheckKind; entityId: string; scheduledAt: string; timeZone: string; createdAt: string; expiresAt: string }
...
export function recheckDedupeKey(request: Pick<TravelRecheckRequest, "tripId" | "kind" | "entityId">): string { return `${request.tripId}:${request.kind}:${request.entityId}`; }
