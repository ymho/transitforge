...
export function previewInTripReplan(trip: Trip, proposal: TripUpdateProposal, input: InTripReplanInput) {
  ...
  for (const patch of proposal.patches) {
    ...
    if (patch.type === "request" || patch.type === "lifecycle" || patch.type === "adoption" || patch.type === "title" || patch.type === "cost_forecast" || patch.type === "cost_override") throw new Error("残り旅程の変更で旅行条件・状態を書き換えられません");
