...
export const responseContractRepairInstruction = "直前の応答は出力contract違反のため表示・保存・実行していません。必要なToolはConverseのnative toolUseだけで呼び出してください。最終JSONはkind=answerまたはkind=askだけとし、本文にTool XML/JSONや内部判断を出さないでください。answer.evidenceIdsには実際に提供済みのEvidence IDだけを指定し、未取得の参照は含めないでください。in-trip回答には有効なinTripAnswerPlanが必要です。";
